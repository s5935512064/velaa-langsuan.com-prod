...
const Gallery = () => {
    ...
    const fetchGallery = async page => {
        ...
        setLoading(true);
        const response = await axios.get(`http://localhost:4500/gallery?page=${page}&per_page=${perPage}&delay=1`);
    ...
    const handlePerRowsChange = async (newPerPage, page) => {
        ...
        const response = await axios.get(`http://localhost:4500/gallery?page=${page}&per_page=${newPerPage}&delay=1`);
        const arraySorted = response.data.results.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
    ...
    const deleteImage = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const deleteURL = `http://localhost:4500/gallery/${id}`
    ...
    const updateStatus = async (id, oldStatus) => {
        ...
        const updateURL = `http://localhost:4500/gallery/${id}`
    ...
    useEffect(() => {
        fetchGallery(1); // fetch page 1 of users
    }, [refresh]);
        ...
        return (
            ...
            <div className="w-full mb-4  h-full">
                ...
                <div className="w-full flex flex-col md:flex-row justify-between gap-2">
                    ...
                    <UploadGallery onUpload={uploadImage} />
                </div>
    ...
    const filteredItems = !filterText ? data :
        data.filter(
            item => item.name.toLowerCase().includes(filterText.toLowerCase()) || item.category.toLowerCase().includes(filterText.toLowerCase())
        );
    ...
    const columns = [
        ...
        {
            name: 'Preview',
            selector: row => <div className="relative py-4 bg-[#fff] loading">

                <div className="image w-20 h-20 relative">
                    <Image
                        href={`http://localhost:4500${row.src}`}
                        data-fancybox={row.id}
                        src={`http://localhost:4500${row.src}`}
                        alt={row.name}
                        loading={"lazy"}
                        ...
                        height={40}
                        objectFit="cover"
                        objectPosition={"center"} className="cursor-pointer" />
                </div>
            </div>,
...

        {
            ...
            selector: row => <div className="inline-flex gap-3">
                ...
                <button onClick={() => deleteImage(row.id)} className="bg-red-500 p-2 text-white rounded text-sm">DELETE</button>
            </div>,
        },
    ];
